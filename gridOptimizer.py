...
class gridOptimizer(object):
    ...
    # 优化gridlayout
    def grid_op(self, ori_row_asses, row_asses, labels, useGlobal=True, useLocal=True, convex_type="E", maxit=10, maxit2=5, only_compact=False):

        start = time.time()
        N = ori_row_asses.shape[0]

        def solve_op(ori_row_asses, row_asses, type, alpha, beta, alter=False, alter_best=None, maxit=5, maxit2=2):
            ...
            if maxit > 0:
                ...
                tmp_row_asses = np.array(gridlayoutOpt.optimizeBA(ori_row_asses, row_asses, labels, change, type, alpha, beta, alter, alter_best, maxit))
                for i in range(N):
            ...
            if maxit2 >= 0:
                ...
                for seed in seed_list:
                    tmp_row_asses = np.array(
                        gridlayoutOpt.optimizeSwap(ori_row_asses, new_row_asses, labels, change, type, alpha, beta,
                                                maxit2, seed, True))
                    for i in range(N):
        ...
        new_cost = np.array([2147483647, 2147483647, 2147483647])
        if useGlobal:
            row_asses1, new_cost1 = solve_op(ori_row_asses, row_asses, "Global", 0, 0, False, None, 0, 0)
            # self.show_grid(row_asses1, labels, square_len, "1.png", False, False)
            print("new_cost1", new_cost1)
            print("time1", time.time()-start)
            row_asses2, new_cost2 = solve_op(ori_row_asses, row_asses, "Global", 0, 1, False, None, global_it, 0)
            ...
            alter_best = [0, 0, 1, 1]
            alter_best[0] = 0
            alter_best[2] = max(new_cost1[0], new_cost2[0])
            ...
            if only_compact:
                ...
                ans, new_cost = solve_op(ori_row_asses, row_asses, "Global", 0.33, 0.33, True, alter_best, global_it, 0)
        ...
        if useLocal:
            ans, new_cost = solve_op(ori_row_asses, ans, convex_type, 1, 0, False, None, maxit, maxit2)
            # if (convex_type == "T") and (new_cost[0] == 0):
            #     print("yes")
            #     ans, new_cost = solve_op(ori_row_asses, ans, convex_type, 1, 0, False, None, 0, 1)
            # self.show_grid(ans, labels, square_len, "5.png", False, False)
    ...
    def grid(self, X_embedded: np.ndarray, labels: np.ndarray = None, type='E', maxit=10, maxit2=5, use_global=True, only_compact=False):
        ...
        # 检查近邻关系变化情况
        def checkNeighborCons(row_asses):
            col_asses = np.zeros(shape=num, dtype='int')
            for i in range(row_asses.shape[0]):
                if row_asses[i] < num:
                    col_asses[row_asses[i]] = i
            _, dist_list = checkNeighbor(ori_row_asses, col_asses)
            tot = dist_list.shape[0]
            per1 = (dist_list <= 1).sum() / tot
            per2 = (dist_list <= 2).sum() / tot
            per3 = (dist_list <= 3).sum() / tot
            per4 = (dist_list <= 4).sum() / tot
            flag = 0
            if per1 >= 0.5:
                flag += 1
            if per2 >= 0.70:
        ...
        ans, t1, t2, new_cost = self.grid_op(ori_row_asses, row_asses, labels, use_global, True, type, maxit, maxit2, only_compact)
